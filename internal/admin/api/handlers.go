...
func (s *Server) Mount(mux *http.ServeMux, base string) {
	...
	mux.HandleFunc(api+"/nebula", s.auth(s.handleNebula))
	mux.HandleFunc(api+"/nebula/ca", s.auth(s.csrf(s.handleNebulaCA)))
	mux.HandleFunc(api+"/nebula/hosts", s.auth(s.csrf(s.handleNebulaHosts)))
	mux.HandleFunc(api+"/nebula/hosts/", s.auth(s.csrf(s.handleNebulaHostID)))
	mux.HandleFunc(api+"/nebula/blocklist", s.auth(s.handleNebulaBlocklist))
