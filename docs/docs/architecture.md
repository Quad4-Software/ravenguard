...
6. **Fleet threat overlay** UA, JA4, DNS, and IP denials from the shared ledger
7. **Optional Q-Feeds cache** for malware IP and domain entries
8. **Optional Coraza** (OWASP CRS) request inspection. When Coraza is enabled in block mode, builtin attack signatures are skipped
9. **Builtin attack signatures** on path/query (when Coraza is off or in detect mode)
10. **Semantic analysis** (optional) decode and intent scoring for SQLi, XSS, CMD, and path
11. **ML score** (optional) pure-Go linear model on structured features, default shadow
12. **Rate limit** keyed by privacy client identity (hashed IP by default). Write methods cost more
13. **Detect** HTTP heuristics, behavior, and merged semantic/ML points (skipped for WebSocket upgrades and allowlisted clients)
14. **Challenge** PoW gate when risk thresholds require it
15. **Access policies** per matched route
...
| Fleet threat overlay | internal/threatshare |
