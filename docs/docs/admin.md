...
| Role | Capabilities |
|------|----------------|
| viewer | Read status, bans, blocklists, threat intel config, config, audit |
| admin | Viewer plus ban management, blocklist reload, threat intel ingest/sync, live config edits, user management (except owners), own API tokens |
...
- Automation can use bearer API tokens (rgat_id.secret). Token secrets are shown once at creation and stored hashed
...
| GET | /threat | fleet threat ledger (redacted) |
| POST | /threat | insert ledger entries and fan-out |
| GET | /threatintel/export.stix | STIX 2.1 bundle (session, API token, or export bearer) |
| GET | /threatintel/export.csv | CSV export |
| POST | /threatintel/ingest | CSV or STIX body into ledger |
| POST | /threatintel/ingest/url | pull one feed URL |
| POST | /threatintel/abuseipdb/sync | AbuseIPDB blacklist |
| POST | /threatintel/abuseipdb/report | report one IP (gated) |
| POST | /threatintel/misp/sync | MISP attribute pull |
| GET/PUT | /threatintel/config | feed settings (keys redacted on GET) |
...
Threat intel details: [Threat intel](./threatintel.md).
