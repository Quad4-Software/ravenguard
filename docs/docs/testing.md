...
1. Request the origin through RavenGuard with a normal browser. Confirm invisible auto-clearance when `mode = "always"` or detect scores trip at low risk.
2. Set `mode = "attack"` and confirm the visible checkbox gate (and captcha when enabled).
